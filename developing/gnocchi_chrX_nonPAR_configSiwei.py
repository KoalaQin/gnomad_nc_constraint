"""
Config Siwei — chrX nonPAR gnocchi pipeline.
Uses Siwei's exact hardcoded "chrX basic" logit coefficients from calculate_methyl_score_x.py,
bypassing the logistic regression step entirely.

Spatial filters match configBasic (= Candidate A from Siwei's active code):
  Mu/element: coverage_mean 23–24, blacklist_gap2 only, logit12

Usage:
  python gnocchi_chrX_nonPAR_configSiwei.py --output-bucket gs://... [step flags]
...
# Siwei's "chrX basic" coefficients (log-odds scale) from calculate_methyl_score_x.py
...
if __name__ == "__main__":
    args = parse_args(description="Gnocchi chrX nonPAR — Config Siwei (hardcoded chrX basic coefficients)")
    run_pipeline(args, CONFIG_SIWEI)